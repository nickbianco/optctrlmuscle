% SolveMuscleRedundancy_NoDynamics, version 0.1 (August 2016)
%
...
function [Time,MExcitation,RActivation,MuscleNames,OptInfo,DatStore]=SolveMuscleRedundancy_NoDynamics(model_path,IK_path,ID_path,time,OutPath,Misc)
...
switch study{2}
    case 'HipAnkle'
        control_bounds_lower = [umin aTmin aDmin];
        control_bounds_upper = [umax aTmax aDmax];
    case 'HipKneeAnkle'
        control_bounds_lower = [umin aTmin aDmin];
        control_bounds_upper = [umax aTmax aDmax];
    case 'HipExtHipAbd'
        ...
        control_bounds_upper = [umax aTmax aDmax];
    otherwise
        control_bounds_lower = [umin aTmin];
        control_bounds_upper = [umax aTmax];
end
...
% States bounds
states_bounds_lower = a_min;
states_bounds_upper = a_max;
...
switch study{2}
    case 'HipAnkle'
        guess.phase.control = [control_guess DatStore.SoRAct./150 0.5*ones(N,1)];
    case 'HipKneeAnkle'
        guess.phase.control = [control_guess DatStore.SoRAct./150 0.5*ones(N,1)];
    case 'HipExtHipAbd'
        guess.phase.control = [control_guess DatStore.SoRAct./150 0.5*ones(N,1)];
    otherwise
        guess.phase.control = [control_guess DatStore.SoRAct./150];
end

guess.phase.state =  DatStore.SoAct(:,1);
