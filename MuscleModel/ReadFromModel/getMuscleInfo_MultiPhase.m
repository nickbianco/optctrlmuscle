function [DatStore] = getMuscleInfo_MultiPhase(IK_path,ID_path,Misc,DatStore,p)
...
% Filter the moment arms information and store them in DatStore(p).dM
...
[B,A] = butter(orde, f_cutoff/(fs/2));
DatStore(p).dM = filtfilt(B,A,dM_raw);

% filter Muscle-tendon lengths and store them in DatStore(p).LMT
...
[B,A] = butter(orde, f_cutoff/(fs/2));
DatStore(p).LMT = filtfilt(B,A,LMT_raw);

% store various informaiton in the DatStore(p) structure
DatStore(p).MuscleNames=Misc.MuscleNames;
DatStore(p).DOFNames=Misc.DofNames;
DatStore(p).nMuscles=length(Misc.MuscleNames);
DatStore(p).nDOF=length(Misc.DofNames);
DatStore(p).q_exp=IK_data.data(IK_inds,DOF_inds+1);        % +1 for time vector
...
DatStore(p).T_exp=ID_data_int(ID_inds,DOF_ID_inds);
