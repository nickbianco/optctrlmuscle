function DatStore = SolveStaticOptimization_IPOPT_MultiPhase(DatStore,p)

% STEP 1. Inputs
% --------------

time = DatStore(p).time;
N = length(time);
M = DatStore(p).nMuscles;
nDof = DatStore(p).nDOF;
...
cos_alpha = ones(N,M);
for m = 1:M
    pp_y = spline(time,DatStore(p).LMT(:,m));
    [LMTg,vMTg,~] = SplineEval_ppuval(pp_y,time,1);
    [~, ~, FMltilde(:,m), FMvtilde(:,m), Fpe(:,m), cos_alpha(:,m)] = HillModel_RigidTendon(act(:,m),LMTg,vMTg,DatStore(p).params(:,m),ActiveFVParameters,PassiveFLParameters,Faparam);
    clear pp_y 
end

% Equations TBC
FMo = ones(size(act,1),1)*DatStore(p).Fiso;
...
ID_data = DatStore(p).T_exp;

I = N*(M+nDof);
MomentArms = zeros(I,nDof);
temp = zeros(N,M);
for i = 1:nDof    
    temp(:,:) = DatStore(p).dM(:,i,:);
...
% xlabel('t [s]')
% ylabel('a_{res} [Nm]')
% legend(DatStore(p).DOFNames);

DatStore(p).SoAct = act;
DatStore(p).SoRAct = eT;
SoForce = FMo.*(act.*FMltilde.*FMvtilde + Fpe); 
DatStore(p).SoForce = SoForce;
